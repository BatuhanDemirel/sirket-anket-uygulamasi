...
// --- CONFIGURATION ---
// Rolleri atamak için e-posta adreslerini buraya girin.
const CONFIG = {
    // Teknik yöneticilerin e-posta adresleri
    ADMIN_EMAILS: ["batuhan.demirel@wesoda.com"], 
    // Anket oluşturacak yöneticilerin e-posta adresleri
    MANAGER_EMAILS: ["serif.kaya@wesoda.com"],
};

// --- FIREBASE SETUP ---
// Sizin tarafınızdan sağlanan Firebase yapılandırması
...
// Kullanıcı profili ve rolü için özel hook
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(userDocRef, (docSnap) => {
            if (docSnap.exists()) {
                ...
                if (!user.email) {
                    console.warn("Attempted to create a profile for a user without an email. Assigning default 'kullanici' role.", user);
                    const newProfile = {
                        email: null,
                        role: 'kullanici',
                ...
                if (CONFIG.ADMIN_EMAILS.includes(userEmail)) {
                    role = 'admin';
                } else if (CONFIG.MANAGER_EMAILS.includes(userEmail)) {
                    role = 'mudur';
                }
                const newProfile = {
                    email: user.email,
                ...
                <div className="w-full max-w-md p-8 space-y-6 bg-gray-800 rounded-lg shadow-lg">
                    ...
                    <form onSubmit={handlePasswordReset} className="space-y-6">
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full px-4 py-2 font-bold text-white bg-blue-600 rounded-md hover:bg-blue-700 disabled:bg-blue-400 transition-colors"
                        >
                    ...
                    <p className="text-sm text-center">
                        <button onClick={() => setIsResetView(false)} className="font-medium text-blue-400 hover:underline">
                            Giriş ekranına dön
                        </button>
                    </p>
                </div>
        ...
        <div className="min-h-screen flex items-center justify-center bg-gray-900">
            <div className="w-full max-w-md p-8 space-y-6 bg-gray-800 rounded-lg shadow-lg">
                ...
                <div className="text-sm text-center text-gray-400 space-y-2">
                    ...
                    {isLogin && (
                        <p>
                            <button onClick={() => { setIsResetView(true); setError(''); setMessage(''); }} className="font-medium text-blue-400 hover:underline">
            ...
            <div className="space-y-8">
                {survey.questions.map((question, qIndex) => {
                    ...
                    surveyAnswers.forEach(answer => {
                        ...
                        if (choice !== undefined && choice < votes.length) {
                            votes[choice]++;
                        }
                    });
